...
def readLoadData(date,id):
    data = pd.read_csv("loadMeasures.csv", sep=';', index_col="datetime", parse_dates=True)
    col = list(data.loc[date,id])
    return col

def readPVData(date):
    data = pd.read_csv("PVforecast-measure.csv", sep=';', index_col="datetime", parse_dates=True)
    col = list(data.loc[date,"PV forecast [kW]"])
    return col
...
if useHistoricLoadPV:
    Pl = readLoadData(date,'59')
    Ps = readPVData(date)
else:
    Pl = readExampleData("dem")
    Ps = readExampleData("PV")
...
if m.status == GRB.Status.OPTIMAL:
    ...
    filename = "OptimalSolution_" + date
    if useHistoricPrice:
